...
// Fake image URLs for static data
	...
	const filteredPets = petList.filter((pet) => {
		for (const key in filters) {
			...
			if (key === "age_stage") {
				...
				// Only show favorites if the user is logged in and the pet is in their favorites
				if (!user.favorites.includes(pet._id)) return false;
			} else {
					...
					filteredPets.map((pet, idx) => {
						return (
							<PetCard
								...
								isFav={user ? user.favorites.includes(pet._id) : false}
								onToggleFav={(event) => toggleFavorite(event, pet._id)}
								key={idx}
